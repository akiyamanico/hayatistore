...
const CatalogCat = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        "https://kmeans-crm-backend-node-c5xdhud6vq-et.a.run.app/produkpicturecat"
      )
      .then((response) => {
  ...
  const urljpg =
    "https://kmeans-crm-backend-node-c5xdhud6vq-et.a.run.app/uploads/";
  ...
  const fetchUsername = async () => {
    try {
      ...
      const userResponse = await axios.get(
        `https://kmeans-crm-backend-node-c5xdhud6vq-et.a.run.app/usermember/${id}`
      );
  ...
  const fetchCat = async () => {
    const response = await axios.get(
      "https://kmeans-crm-backend-node-c5xdhud6vq-et.a.run.app/categorylist"
    );
  ...
  const handleLogout = async () => {
    try {
      ...
      await axios.post(
        "https://kmeans-crm-backend-node-c5xdhud6vq-et.a.run.app/logout"
      );
            ...
            <Link to={`/ProductView/${product.id_produk}`}>
              <div class="relative max-w-sm min-w-[340px] bg-white shadow-md rounded-3xl p-2 mx-1 my-3 cursor-pointer">
                ...
                <div class="mt-4 pl-2 mb-2 flex justify-between ">
                  <div>
                    <p class="text-lg font-creato text-gray-900 mb-0">
                      {product.nama}
                    </p>
